...
export const Table = ({ table, show, locale, search, unfilteredRows, handleDelete, handleUndo, pageSize = 7 }) => {
    ...
    useLayoutEffect(() => {
        ...
        if (ref.current == null || innerRef.current == null)
            return;
    ...
    function renderRow(item, i) {
        ...
        return (_jsxs("tr", Object.assign({ className: 'border-b-2 border-grey4 border-solid' }, { children: [_jsx("td", { children: _jsx(CheckBox, { id: item.id, size: 'w-6 h-6', selected: selected.has(item.id), onSelect: () => toggleSelected(item.id) }) }, 'select'), item.cells.map((cell, j) => (_jsx("td", { children: _jsx(Cell, { cell: cell, search: search, cellClass: cellClass, setTooltip: setTooltip }) }, j)))] }), item.id));
    }
    ...
    return (_jsx("div", Object.assign({ ref: ref, className: 'grid grid-cols-1 transition-[grid,color] duration-500 relative overflow-hidden text-sm md:text-base' }, { children: _jsxs("div", Object.assign({ ref: innerRef, className: `h-min ${unfilteredRows === 0 ? 'invisible' : ''}` }, { children: [_jsxs("div", Object.assign({ className: 'my-2 bg-grey6 rounded-md border-grey4 border-[0.2rem]' }, { children: [_jsx("div", Object.assign({ className: 'p-3 pt-1 pb-2 max-w-full overflow-x-scroll' }, { children: _jsxs("table", Object.assign({ className: 'table-fixed min-w-full ' }, { children: [_jsx("thead", Object.assign({ className: '' }, { children: _jsxs("tr", Object.assign({ className: 'border-b-2 border-grey4 border-solid' }, { children: [_jsx("td", Object.assign({ className: 'w-8' }, { children: _jsx(CheckBox, { id: 'selectAll', size: 'w-6 h-6', selected: table.body.rows.length > 0 && selected.size === table.body.rows.length, onSelect: toggleSelectAll }) })), columnNames.map(renderHeaderCell)] })) })), _jsx("tbody", { children: items.map(renderRow) })] })) })), _jsxs("div", Object.assign({ className: 'px-3 pb-1 flex justify-between min-h-[2.5rem]' }, { children: [_jsx("div", Object.assign({ className: 'pt-2 pb-2' }, { children: selected.size > 0 || table.deletedRowCount === 0
                                        ? (_jsx(IconButton, { icon: DeleteSvg, label: `${text.delete} ${selected.size > 0 ? selectedLabel : ''}`, color: 'text-delete', disabled: selected.size === 0, onClick: () => handleDelete === null || handleDelete === void 0 ? void 0 : handleDelete([...selected]) }))
...
function IconButton(props) {
    ...
    return (_jsxs("div", Object.assign({ className: `flex items-center gap-2 cursor-pointer  ${props.color} animate-fadeIn md:text-button ${disabled ? 'opacity-50' : ''}`, onClick: () => !disabled && props.onClick() }, { children: [_jsx("img", { src: props.icon, className: 'w-7 h-7 ml-1 md:w-9 md:h-9 md:ml-0 -translate-x-[3px]' }), props.label] })));
}
